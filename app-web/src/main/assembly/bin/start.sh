...
SERVER_NAME=`sed '/cooas.application.name/!d;s/.*=//' "$CONF_DIR/cooas.properties" | tr -d '\r'`
SERVER_HOST=`sed '/cooas.jetty.host/!d;s/.*=//' "$CONF_DIR/cooas.properties" | tr -d '\r'`
SERVER_PORT=`sed '/cooas.jetty.port/!d;s/.*=//' "$CONF_DIR/cooas.properties" | tr -d '\r'`
SERVER_JMX_PORT=`sed '/cooas.jmxport/!d;s/.*=//' "$CONF_DIR/cooas.properties" | tr -d '\r'`
SERVER_DEBUG_PORT=`sed '/cooas.debugport/!d;s/.*=//' "$CONF_DIR/cooas.properties" | tr -d '\r'`
...
if [ -z "$SERVER_JMX_PORT" ]; then
    SERVER_JMX_PORT=$(($SERVER_PORT + 10))
fi

if [ -z "$SERVER_DEBUG_PORT" ]; then
    SERVER_DEBUG_PORT=$(($SERVER_PORT + 20))
fi
...
CLASS_PARAM="-Dcooas.jetty.webappPath=$DEPLOY_DIR/webapp com.xsoft.appdemo.jetty.JettyServer"

echo -e "Starting the $SERVER_NAME ...\c"
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS \
    -classpath $CONF_DIR:$LIB_DIR:$LIB_JARS \
    -Dhscooas.home="$DEPLOY_DIR" \
